import { RANKS, SUITS, type Card } from "../../engine/cards";
import type { ParticleScenario, CanonicalInitialDeal, HiddenTransferAssignment } from "./contracts";
...
import { validateParticleBankPublic } from "./particleBankPublicValidation";
...
export function readParticleBankRolloutAccess(bank: unknown): ParticleBankRolloutAccessResult {
  try {
    const publicValidation = validateParticleBankPublic(bank);
    if (!publicValidation.ok) return unknownBankFailure();
    const validatedBank = publicValidation.bank;
...
function isSeat(value: unknown): value is 0 | 1 | 2 | 3 {
  return typeof value === "number" && Number.isSafeInteger(value) && !Object.is(value, -0) && (value === 0 || value === 1 || value === 2 || value === 3);
}
