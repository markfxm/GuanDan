...
import { buildD1ManifestV2, validateResumeManifestV2 } from "../tests/benchmark/d1Manifest";
...
import { strategyDescriptorsForExecution } from "../tests/benchmark/strategies";
...
import { deriveD1ConfigHash, resolveGitExecutionProvenance, type GitCommitSha } from "../tests/benchmark/d1Provenance";
import { buildD1ExecutionProvenance, hashD1ExecutionProvenance } from "../tests/benchmark/d1ProvenanceV2";

export interface D1RunnerOptions { phase: D1Phase; matchup?: string; seedStart?: number; seedEnd?: number; replayMode?: "none" | "failures" | "all"; resume: boolean; skipExisting: boolean; concurrency: number; dryRun: boolean; outputDir: string; configHash: string; requestedConfigHash: string; configHashFrozen: boolean; executionSourceCommit: GitCommitSha; worktreeClean: boolean; help?: boolean; approval?: FormalApproval; }
...
export function parseD1Args(args: string[]): D1RunnerOptions {
  ...
  const requestedConfigHash = values.get("config-hash")?.value ?? "d1-config-unfrozen";
  if (requestedConfigHash.length === 0) throw new Error("CONFIG_HASH_INVALID");
  const provenance = resolveGitExecutionProvenance({ cwd: process.cwd() });
  const configHash = deriveD1ConfigHash({ requestedConfigHash, executionSourceCommit: provenance.executionSourceCommit, benchmarkVersion: "d1-topk-v1", rank: "2", phase, replayMode });
  return { phase, matchup: values.get("matchup")?.value, seedStart: start, seedEnd: end, replayMode, resume: flags.has("resume"), skipExisting: flags.has("skip-existing"), concurrency, dryRun: flags.has("dry-run"), outputDir, configHash, requestedConfigHash, configHashFrozen: values.get("config-hash") !== undefined, executionSourceCommit: provenance.executionSourceCommit, worktreeClean: provenance.worktreeClean, help: flags.has("help"), approval: undefined };
}
...
export function buildD1DryRunOutput(options: D1RunnerOptions): D1DryRunOutput {
  ...
  return {
    ...
    executionSourceCommit: options.executionSourceCommit,
...
export async function runD1(options: D1RunnerOptions): Promise<{ dryRun: boolean; plan: ReturnType<typeof planD1Run>; dryRunOutput?: D1DryRunOutput; manifest?: unknown }> {
  ...
  if (!options.worktreeClean) throw new Error("EXECUTION_WORKTREE_DIRTY");
  if (!options.configHashFrozen && options.phase === "formal") throw new Error("FORMAL_CONFIG_NOT_FROZEN");
  if (options.phase === "formal") { const approval = validateFormalApproval(options.approval, { expectedConfigHash: options.configHash, currentCommit: options.executionSourceCommit, currentTreeHash: computeApprovedCodeTreeHash(), }); if (approval.formalExecutionAllowed !== true) throw new Error("FORMAL_EXECUTION_NOT_ALLOWED"); }
  ...
  const descriptors = strategyDescriptorsForExecution(options.executionSourceCommit);
  const provenance = buildD1ExecutionProvenance({ executionSourceCommit: options.executionSourceCommit, configHash: options.configHash, phase: options.phase, seedStart: options.seedStart!, seedEnd: options.seedEnd!, strategyDescriptors: descriptors, implementationVersion: "dynamic-topk-v1", engineVersion: ENGINE_VERSION, roomRulesVersion: ROOM_RULES_VERSION, keepCurrentLockFixtureHash: process.env.D1_KEEP_CURRENT_LOCK_FIXTURE_HASH });
  const provenanceHash = hashD1ExecutionProvenance(provenance);
  ...
  for (const range of ranges) { if (range.start > range.end) continue; const expected = expectedIdsForRange(range.matchup.name, range.start, range.end, options.configHash); const batchPath = path.join(options.outputDir, `${safeName(range.batchId)}.json`); const manifestPath = `${batchPath}.manifest.json`; if (options.resume) { try { const previous = JSON.parse(await fs.readFile(manifestPath, "utf8")); const nextShape = { schemaVersion: "d1-manifest-v2", phase: options.phase, matchup: range.matchup.name, configHash: options.configHash, executionSourceCommit: options.executionSourceCommit, provenanceHash, diagnosticsIntegrity: { integrityOk: true }, executionProvenance: provenance }; if ((previous.expectedMatchIds ?? []).join("\n") !== expected.join("\n")) throw new Error("EXPECTED_MATCH_IDS_MISMATCH"); validateResumeManifestV2(previous, nextShape as never); if (options.skipExisting && (previous.completedMatchIds ?? []).length === expected.length) { lastManifest = previous; continue; } } catch (error) { if (error instanceof Error && ["CONFIG_HASH_MISMATCH", "EXECUTION_SOURCE_COMMIT_MISMATCH", "EXPECTED_MATCH_IDS_MISMATCH", "LEGACY_MANIFEST_NOT_RESUMABLE", "PROVENANCE_HASH_MISMATCH", "DIAGNOSTICS_INTEGRITY_INVALID"].includes(error.message)) throw error; } } const batch = await runBatch(range.matchup, range.start, range.end, options, provenance); const manifest = buildD1ManifestV2({ phase: options.phase, matchup: range.matchup.name, batchId: range.batchId, configHash: options.configHash, expectedMatchIds: expected, games: batch.games, executionProvenance: provenance, provenanceHash, strategyDescriptors: descriptors, replayMode: options.replayMode! }); await writer.writeBatch(batchPath, `${JSON.stringify(batch.games, null, 2)}\n`, manifestPath, `${JSON.stringify(manifest, null, 2)}\n`); lastManifest = manifest; }
  return { dryRun: false, plan, manifest: lastManifest };
}

async function runBatch(matchup: { name: string; strategyA: string; strategyB: string }, start: number, end: number, options: D1RunnerOptions, provenance: ReturnType<typeof buildD1ExecutionProvenance>): Promise<{ games: ReturnType<typeof simulateGame>[] }> { const games: ReturnType<typeof simulateGame>[] = []; const descriptors = strategyDescriptorsForExecution(options.executionSourceCommit); for (let seed = start; seed <= end; seed += 1) { const config = { benchmarkVersion: "d1-topk-v1", rank: "2" as const, seeds: [seed], strategyA: matchup.strategyA, strategyB: matchup.strategyB, replayMode: options.replayMode!, executionSourceCommit: options.executionSourceCommit }; for (const originalTask of buildGamesForSeed(config, seed)) { const task = { ...originalTask, phase: options.phase, matchup: matchup.name, executionProvenance: provenance, configHash: options.configHash, executionSourceCommit: options.executionSourceCommit, matchId: expectedMatchId({ matchup: matchup.name, seed, allocation: originalTask.allocation, rotation: originalTask.rotation, configHash: options.configHash }) }; const result = simulateGame(task); games.push(result); if (options.replayMode === "all" || (options.replayMode === "failures" && result.failed)) writeD1Replay(result, path.join(options.outputDir, "replays", matchup.name), { benchmarkVersion: "d1-topk-v1", replayVersion: "d1-replay-v1", engineVersion: ENGINE_VERSION, roomRulesVersion: ROOM_RULES_VERSION, strategyDescriptors: descriptors }); } } return { games }; }
