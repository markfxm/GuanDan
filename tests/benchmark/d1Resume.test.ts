...
describe("D1 resume and skip-existing", () => {
  it("only skips a complete safety-valid result with matching provenance", () => {
    const base = { matchId: "m1", configHash: "cfg", implementationVersion: "impl", completed: true, failed: false, durationMs: 1, diagnosticsError: false, publicTraceHash: "h", replayVerified: true };
    expect(canSkipExisting(base, { configHash: "cfg", implementationVersion: "impl", replayMode: "all" })).toBe(true);
  ...
  it("rejects merging batches with config drift or missing IDs", () => {
    const base = { schemaVersion: "d1-manifest-v1" as const, phase: "formal", matchup: "treatment-vs-control", configHash: "a", expectedMatchIds: ["m1"], completedMatchIds: ["m1"], resumeSupported: true as const, skipExistingSupported: true as const };
